...
test('it should return decorators with parsed options', () => {
    expect(getExperimentalParserPlugins(['["decorators", { "decoratorsBeforeExport": true }]'])).toEqual([
        ['decorators', { decoratorsBeforeExport: true }],
    ]);
});

test('it should return decorators with parsed options', () => {
    expect(getExperimentalParserPlugins(['flow', '["decorators", { "decoratorsBeforeExport": true }]'])).toEqual([
        'flow',
        ['decorators', { decoratorsBeforeExport: true }],
    ]);
});
...
test('it should throw an Error for invalid JSON', () => {
    expect(() =>
        getExperimentalParserPlugins(['flow', '["decorators", { decoratorsBeforeExport: true }]']),

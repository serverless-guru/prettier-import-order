...
it('should merge duplicate imports within a given chunk', () => {
    ...
    expect(format(formatted, { parser: 'babel' })).toEqual(`import type { A, B } from "a";
import { Junk } from "junk-group-1";
...
it("doesn't merge duplicate imports if option disabled", () => {
    ...
    expect(format(formatted, { parser: 'babel' })).toEqual(`import type { A } from "a";
import type { B } from "a";
import { Junk } from "junk-group-1";
...
it('merges named and default imports correctly', () => {
    ...
    expect(format(formatted, { parser: 'babel' })).toEqual(`import A, { a, ahh } from "a";
import B, { bee } from "b";
`);
});

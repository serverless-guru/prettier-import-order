...
import { importFlavorType, importFlavorValue, mergeableImportFlavors } from '../constants';
 ...
 */
function selectMergeableNodesByImportFlavor(nodes: ImportDeclaration[]): Record<MergeableFlavor, ImportDeclaration[]> {
    return nodes.reduce<Record<typeof mergeableImportFlavors[number], ImportDeclaration[]>>(
        (groups, node) => {
...
function mergeIsSafe(nodeToKeep: ImportDeclaration, nodeToForget: ImportDeclaration) {
    if (
 ...
 */
function mergeNodes(nodeToKeep: ImportDeclaration, nodeToForget: ImportDeclaration) {
    ...
    nodeToKeep.leadingComments = [...(nodeToKeep.leadingComments || []), ...(nodeToForget.leadingComments || [])];
    nodeToKeep.innerComments = [...(nodeToKeep.innerComments || []), ...(nodeToForget.innerComments || [])];
    nodeToKeep.trailingComments = [...(nodeToKeep.trailingComments || []), ...(nodeToForget.trailingComments || [])];
 ...
 */
export const mergeNodesWithMatchingImportFlavors: MergeNodesWithMatchingImportFlavors = (input) => {
    const nodesToDelete: ImportDeclaration[] = [];

    for (const group of Object.values(selectMergeableNodesByImportFlavor(input))) {

import { CommentBlock, CommentLine, Directive, ImportDeclaration, InterpreterDirective, Statement } from '@babel/types';
...
function hasRange(range: OptionalRange): range is Range {
    return range[0] !== null && range[1] !== null && Number.isSafeInteger(range[0]) && Number.isSafeInteger(range[1]);
}
...
export const removeNodesFromOriginalCode = (
    code: string,
    nodes: (Statement | CommentBlock | CommentLine | ImportDeclaration | InterpreterDirective | Directive)[],
): string => {
    ...
    const excludes = nodes
        .map((node) => [node.start, node.end] as OptionalRange)
        .filter(hasRange)
    ...
    return includes.map((include) => code.substring(include[0], include[1])).join('');
};
